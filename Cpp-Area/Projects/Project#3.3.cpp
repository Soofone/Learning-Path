...
bool WithdrawlientByAccountNumber(string AccountNumber, vector <sClient>& vClients)
{
    sClient Client;
    char Answer = 'n';
    short Amount = 0;
    if (FindClientByAccountNumber(AccountNumber, vClients, Client))
    {
        ...
        if (Amount > Client.AccountBalance)
        {
            do
            {
                cout << "\n\nAmount Exceeds the balance, you wan withdraw up to: " << Client.AccountBalance;
...
void ShowWithdrawScreen()
{
    ...
    vector <sClient> vClients = LoadCleintsDataFromFile(ClientsFileName);
    string AccountNumber = ReadClientAccountNumber();
    WithdrawlientByAccountNumber(AccountNumber, vClients);
}

void PrintTotalBalancesClientRecordLine(sClient Client)
{
    cout << "| " << setw(15) << left << Client.AccountNumber;
    cout << "| " << setw(40) << left << Client.Name;
    cout << "| " << setw(12) << left << Client.AccountBalance;
...
void ShowTotalBalances()
{
    ...
    if (vClients.size() == 0)
        ...
        for (sClient Client : vClients)
        {
            ...
            PrintTotalBalancesClientRecordLine(Client);
            cout << endl;
        }
